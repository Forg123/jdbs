...
public class PublicBaseController {
    ...
    public Result getRoleMenus(@RequestBody Input input) throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        Result result = new Result();
        String roleId = (String) input.getParams().get("roleId");
        if (roleId == null || roleId == ""){
            input.getParams().put("roleId" , userInfo.getRoles());
        }
